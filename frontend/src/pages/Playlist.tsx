...
export default function PlaylistPage() {
  ...
  async function generateChart(
    donut: Record<string, { artist: string; count: number }>,
  ) {
          ...
          datasets: [
            {
              ...
              backgroundColor: ["#FF6384", "#36A2EB", "#FFCE56", "#8B008B"],
            },
          ],
        },
        options: {
  ...
  return (
    <div className="bg-gray-900">
      <h1 className="text-white font-bold text-4xl bold tracking-normal text-center mb-5">
        {playlistName}
      </h1>
      <header className="h-[800px] w-full flex items-center justify-center">
        {loading ? (
          <h1 className="text-white text-4xl tracking-normal text-center mb-5">
            Loading...
          </h1>
        ) : (
          <canvas
        ...
        {tracks ? (
          tracks.items.map((obj) => (
            <div key={obj.track.id} className="flex flex-col w-full mb-2.5">
              <button
                onClick={() => {
                  window.open(obj.track.external_urls.spotify, "_blank");
                }}
              >
                <img
                  className="block w-full rounded-lg shadow-lg border-2 border-black hover:border-green-500"
                  key={obj.track.id}
